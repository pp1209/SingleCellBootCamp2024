...
obj
```
This normalization set a scale factor of 10,000 for each cell (so the sum of the counts of a cell is set to 10,000), and adjust the expression value of each gene accordingly. It then calculates the log1p of those values (natural logarithm of the value+1 to account for zeros).
...
```{r}
obj@reductions$pca@cell.embeddings[1:10, 1:20]
...
DimPlot(obj, reduction = "umap.unintegrated", group.by = c("Method", "CellType"))
```
...
DimPlot(obj, reduction = "umap.cca", group.by = c("Method", "Experiment"))
```
...
```
Inspecting this data, we still need to integrate
```{r}
