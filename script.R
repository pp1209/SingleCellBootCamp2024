...
#Scale data
# Data scaling is fundamental to 
...
# Since by default the active identity is the "responders" vs "non responders", the PCA plot will display the two categories. We might want to change the active identity to the samples, as displayed below
...
# How many PCs to include in your clustering? Using JackStraw: randomly permutes genes and calculates PC assigning a score, then compares these scores with query data and assigns a p-value
...
cells_clust_tree <- FindClusters(tcells, resolution = seq(from=0, to=2, by=0.1), print.output = 1, save.SNN = T)
...
tcells.markers %>% group_by(cluster) %>% slice_max(n = 6, order_by = avg_log2FC)
