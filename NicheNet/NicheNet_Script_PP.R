...
setwd("\\\\unicloud.unimi.it/share/Home/pierangela.palmerini/Desktop/Data")
...
## Sender: MAIT cells
Idents(MAIT) = 'time'
# Select only genes that are expresseed by MAIT cells at timepoint 3. Select genes that are expresssed at least by 10% of cells in the cluster (pct = 0.1)
...
### Define a gene set of potential ligands
  # These are ligands that are expressed by the sender cell population (MAIT cells) and bind a putative receptor expressed by the receiver population (B cells)
ligands =  lr_network %>% pull(from) %>% unique()
receptors = lr_network %>% pull(to) %>% unique()

# select all ligands expressed by sender and that are present also in the matrix
expressed_ligands = intersect(ligands, expressed_genes_sender)
expressed_receptors =  intersect(receptors, expressed_genes_receiver)

# select all ligands that have an interaction with receiver cells
...
### Rank the potential ligands based on the presence of their target genes in the gene set of interest
  # This Pearson correlation indicates the ability of each ligand to predict differential expressed genes P3 vs P0, and better predictive ligands are thus ranked higher.
...
#The function get_weighted_ligand_target_links will return genes that are in the gene set of interest and are the top n targets of a ligand. NB: the "weight" parameter will not be based on your dataset but on the literature (same value displayed in the ligand-receptor matrix)
active_ligand_target_links_df = best_upstream_ligands %>% lapply(get_weighted_ligand_target_links, geneset = geneset_oi, ligand_target_matrix = ligand_target_matrix, n = 200) %>% bind_rows() %>% drop_na()
head(active_ligand_target_links_df)

## To visualize as a heatmap matrix:
#The ligand-target matrix between the top 20 prioritized MAIT ligands expressed at time point P3and the predicted target genes expressed by the B cells, where the matrix is colored according to the regulatory potential values.
...
#The darker the purple, the stronger the interaction is.
